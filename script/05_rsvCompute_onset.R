...
rsv_onset_temp <-
  bind_rows(onset.samples_temp, .id = "id") %>%
  ...
  summarise(epiwk = mean(onset, na.rm = TRUE),
...
#compute mean, low and upper 95%CI for onset by country and preCOVID-19 vs each year after COVID-19 year
rsv_onset_trop <-
  bind_rows(onset.samples_trop, .id = "id") %>%
...
rm("Gmodels", "onset.samples", "W", "X", "dtime", "t", "i", "set.seed", "deriv", "middle_pts", "onset.samples_temp", "onset.samples_trop", "cases.samples")
